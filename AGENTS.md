...
GitHub var `VITE_BACKEND_URL` 已設 → 使用端零設定走 proxy 模式。

待：

1. 請泰語母語者核對 `src/data.js` 術語／句庫（推廣前必做）
2. 輪替舊的 Gemini API key（前端已不再持有 key，改走 GAS proxy）
...
- 術語與常用句資料在 `src/data.js`：81 條壓鑄術語 + 69 句預譯常用句（0 API、離線可用）。
